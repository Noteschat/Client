...
import 'package:noteschat/components/NotesSelect/NotesTags.dart';
            ...
            onTap: () {
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder:
                      (context) =>
                          NoteView(allNote: note, chatId: chatId, host: host),
                ),
              );
            },
            onLongPress: () {
              ...
              style: ButtonStyle(
                backgroundColor: WidgetStateColor.resolveWith((states) {
                  if (states.contains(WidgetState.pressed)) {
                    return Theme.of(context).colorScheme.error.withValues(
                      alpha: 0.7,
                    ); // Error color when pressed
                  }
                  return Theme.of(
                    context,
                  ).colorScheme.error; // Default error color
                }),
              ),
              onPressed: () async {
                await http.delete(
                  Uri.parse(
                    "http://${host}/api/notes/storage/${chatId}/${note.id}",
                  ),
                  headers: headers,
                );
                removeNote();
